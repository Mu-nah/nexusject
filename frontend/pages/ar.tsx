...
const gbp = (n: number) => `£${n.toLocaleString('en-GB', { minimumFractionDigits: 0 })}`
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: 0 }}>
        {(['invoices', 'customers', 'aged', 'receipts', 'credit'] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: '8px 16px', border: 'none', cursor: 'pointer', fontSize: 12.5,
            background: 'none', borderBottom: tab === t ? '2px solid #C9A84C' : '2px solid transparent',
            color: tab === t ? '#E8C56A' : '#5C6B84', fontWeight: tab === t ? 600 : 400,
            fontFamily: "'Instrument Sans', sans-serif", textTransform: 'capitalize',
          }}>
            {t === 'invoices' ? 'Invoices' : t === 'customers' ? 'Customers' : t === 'aged' ? 'Aged Debtors' : t === 'receipts' ? 'Receipts' : 'Credit Control'}
          </button>
        ))}
      </div>
          ...
          <Panel noPadding>
            <DataTable
              columns={[
                ...
                { key: 'customer', header: 'Customer', render: r => <span style={{ fontWeight: 500, color: '#E8EDF5' }}>{r.customer}</span> },
      ...
      {tab === 'customers' && (
        <>
          ...
          <Panel noPadding>
            <DataTable
              columns={[
                { key: 'name', header: 'Customer', render: r => <span style={{ fontWeight: 500, color: '#E8EDF5' }}>{r.name}</span> },
                { key: 'type', header: 'Type', render: r => <Badge variant="slate">{r.type}</Badge> },
          ...
          <Panel noPadding>
            <DataTable
              columns={[
                ...
                { key: 'customer', header: 'From', render: r => <span style={{ fontWeight: 500, color: '#E8EDF5' }}>{r.customer}</span> },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <Panel title="Credit Control Actions" titleIcon="⊟" iconColor="#FB8C00">
            <DataTable
              columns={[
                { key: 'customer', header: 'Customer', render: r => <span style={{ fontWeight: 500, color: '#E8EDF5' }}>{r.customer}</span> },
                { key: 'amount', header: 'Amount', align: 'right', mono: true },
                { key: 'action', header: 'Action', render: r => <Badge variant={r.aVariant as any}>{r.action}</Badge> },
              ]}
              data={[
          ...
          <Panel title="Chasing Schedule" titleIcon="◷" iconColor="#5E9EFF">
            {[
              ...
              <div key={s.step} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: s.color, flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: '#C8D3E8' }}>{s.step}</div>
                  <div style={{ fontSize: 11, color: '#5C6B84' }}>{s.trigger} · {s.method}</div>
                </div>
              </div>
            ))}
          </Panel>
