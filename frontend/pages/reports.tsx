...
function renderInline(text: string): ReactNode[] {
  ...
  return parts.filter(Boolean).map((part, index) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return (
        <strong key={index} style={{ color: 'var(--heading)', fontWeight: 700 }}>
          {part.slice(2, -2)}
        </strong>
      )
    }
...
function renderMarkdownReport(markdown: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (line === '---') {
      elements.push(<hr key={`hr-${i}`} style={{ border: 0, borderTop: '1px solid var(--line)', margin: '24px 0' }} />)
      i += 1
      continue
    }
    ...
    if (line.startsWith('# ')) {
      elements.push(<h1 key={`h1-${i}`} style={{ fontSize: 28, lineHeight: 1.15, color: 'var(--heading)', margin: '0 0 18px', fontWeight: 700, letterSpacing: '-0.03em' }}>{renderInline(line.slice(2))}</h1>)
      i += 1
      continue
    }
    if (line.startsWith('## ')) {
      elements.push(<h2 key={`h2-${i}`} style={{ fontSize: 20, lineHeight: 1.25, color: 'var(--heading)', margin: '28px 0 12px', fontWeight: 700 }}>{renderInline(line.slice(3))}</h2>)
      i += 1
      continue
    }
    if (line.startsWith('### ')) {
      elements.push(<h3 key={`h3-${i}`} style={{ fontSize: 16, lineHeight: 1.3, color: 'var(--heading)', margin: '22px 0 10px', fontWeight: 700 }}>{renderInline(line.slice(4))}</h3>)
      i += 1
      continue
    }
    if (line.startsWith('#')) {
      ...
      elements.push(
        <h3
          key={`hx-${i}`}
          style={{ fontSize: 15, lineHeight: 1.35, color: 'var(--heading)', margin: '20px 0 10px', fontWeight: 700 }}
        >
      ...
      elements.push(
        ...
        >
          {metaSegments.map((segment, segmentIndex) => (
            ...
            >
              <div style={{ fontSize: 10.5, color: 'var(--mute2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>{segment.label}</div>
              <div style={{ fontSize: 13.5, color: 'var(--heading)', lineHeight: 1.6 }}>{renderInline(segment.value)}</div>
            </div>
          ))}
        </div>
      )
    ...
    if (line.startsWith('>')) {
      ...
      elements.push(
        <blockquote key={`quote-${i}`} style={{ margin: '18px 0', padding: '14px 16px', borderLeft: '3px solid var(--amber)', background: 'var(--amber-bg)', borderRadius: 10, color: 'var(--heading)' }}>
    ...
    if (/^- /.test(line)) {
      ...
      elements.push(
        <ul key={`ul-${i}`} style={{ margin: '10px 0 18px 18px', padding: 0, color: 'var(--text)' }}>
    ...
    if (paragraphLines.length === 0) {
      elements.push(<p key={`fallback-${i}`} style={{ margin: '0 0 16px', fontSize: 14, lineHeight: 1.85, color: 'var(--text)' }}>{renderInline(line)}</p>)
      i += 1
      continue
    }

    elements.push(<p key={`p-${i}`} style={{ margin: '0 0 16px', fontSize: 14, lineHeight: 1.85, color: 'var(--text)' }}>{renderInline(paragraphLines.join(' '))}</p>)
  }
    ...
    <AppLayout title="Financial Reports" subtitle="Workspace library">
      {shareQuery && sharedAccessRequired && !report && (
        <Panel style={{ marginBottom: 18 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--mute2)', lineHeight: 1.7, marginBottom: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 24 }}>
        {REPORTS.map((r) => (
          <Panel key={r.id} style={{ position: 'relative', overflow: 'hidden', minHeight: 184 }}>
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at top right, var(--gold-bg), transparent 40%)', pointerEvents: 'none' }} />
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, position: 'relative' }}>
              <div style={{ flexShrink: 0, width: 52, height: 52, borderRadius: 14, background: 'var(--bg3)', border: '1px solid var(--line2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold2)', fontSize: 12, fontWeight: 700 }}>
                {r.icon}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 18, alignItems: 'start' }}>
        ...
        {filteredHistory.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--mute2)' }}>Generate your first workspace report to start building the shared library.</div>
        ) : (
...
{visibleHistory.map((rep) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: isActive ? '1px solid var(--gold-line)' : '1px solid var(--line)',
                      borderRadius: 14,
                    }}
                  >
                    <div
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                        <div style={{ fontWeight: 600, color: 'var(--heading)' }}>{rep.title}</div>
                        <Badge variant="slate">{rep.report_type.replace(/_/g, ' ')}</Badge>
                      </div>
                      <div style={{ fontSize: 11.5, color: 'var(--mute2)' }}>
      ...
      {shareModalOpen && shareTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 230, padding: 18 }}>
          <div style={{ width: '100%', maxWidth: 560, background: 'var(--bg2)', border: '1px solid var(--line2)', borderRadius: 22, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '18px 22px', borderBottom: '1px solid var(--line)', background: 'var(--bg3)' }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--heading)', marginBottom: 4 }}>Share Report</div>
                <div style={{ fontSize: 12.5, color: 'var(--mute2)' }}>{shareTarget.title}</div>
              </div>
              <button
                type="button"
                onClick={() => setShareModalOpen(false)}
                style={{ width: 38, height: 38, borderRadius: 12, border: '1px solid var(--line2)', background: 'var(--bg2)', color: 'var(--text)', cursor: 'pointer', fontSize: 16 }}
              >
                X
              </button>
            </div>
      ...
      {viewerOpen && report && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'stretch', justifyContent: 'center', zIndex: 220, padding: 18 }}>
          <div style={{ width: '100%', maxWidth: 1280, background: 'var(--bg2)', border: '1px solid var(--line2)', borderRadius: 22, overflow: 'hidden', display: 'grid', gridTemplateRows: 'auto auto 1fr', maxHeight: 'calc(100vh - 36px)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '18px 22px', borderBottom: '1px solid var(--line)', background: 'var(--bg3)', flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--heading)', marginBottom: 6 }}>{report.title}</div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <Badge variant="green">Saved to Workspace</Badge>
                  <Badge variant="slate">{report.report_type.replace(/_/g, ' ')}</Badge>
                  {report.period_label && <Badge variant="blue">{report.period_label}</Badge>}
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{ width: 38, height: 38, borderRadius: 12, border: '1px solid var(--line2)', background: 'var(--bg2)', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}
                >
                  ×
                </button>
              </div>
            ...
            <div style={{ overflowY: 'auto', padding: '26px 28px 32px', background: 'linear-gradient(180deg, var(--bg) 0%, var(--bg3) 100%)', fontFamily: 'Instrument Sans, sans-serif' }}>
              <div style={{ maxWidth: 980, margin: '0 auto', background: 'var(--bg2)', border: '1px solid var(--line2)', borderRadius: 20, padding: 'clamp(18px, 3vw, 30px)', boxShadow: 'var(--shadow)' }}>
                {fullReportElements}
              </div>
            </div>
          </div>
